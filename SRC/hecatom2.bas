...
IF code$ = code9$ THEN
	...
	FOR dry = 1 TO ymax
		FOR drx = 1 TO xmax
			IF feld(drx, dry) = 1.1 THEN what = 0: GOSUB drawobjs
			IF feld(drx, dry) = 222 THEN what = 22.22 ELSE what = feld(drx, dry)
			GOSUB drawobjs
		NEXT
	NEXT
	drx = x
...
PUT ((drx * big) - big + 1, (dry * big) - big + 1), money, PSET
	...
	PSET ((drx * big) - (big / 2) + 1, (dry * big) - (big / 2) + 4), 14
	PSET ((drx * big) - (big / 2), (dry * big) - (big / 2) + 5), 14
...
restartnew:
oldscore = oldscore + score
OPEN "heca" + level$ + ".lvl" FOR INPUT AS #1
INPUT #1, remark$
INPUT #1, xmax, ymax
REDIM feld(xmax, ymax)
FOR xx = 1 TO xmax
